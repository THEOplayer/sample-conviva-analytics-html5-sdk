...
function collectContentMetdata(player, configuredContentMetadata) {
    return Object.assign({ [Conviva.Constants.STREAM_URL]: player.src, [Conviva.Constants.PLAYER_NAME]: 'THEOplayer', [Conviva.Constants.DURATION]: player.duration }, configuredContentMetadata);
}
...
window.__logComponents = window.__logComponents || [];
function log (component, message) {
    if (window.__logComponents.indexOf(component) === -1) {
...
class NewConvivaIntegration {
    constructor(player, convivaConfiguration, contentMetadataReceiver) {
        ...
        this._onPlayerLoadedMetadata = () => {
            ...
            if (!this._isPlayingAd && this._convivaVideoAnalytics) {
                ...
                if (!isNaN(duration) && duration !== Infinity) {
                    ...
                    log('VideoAnalytics', `setContentInfo(${contentInfo})`);
                }
                ...
                log('VideoAnalytics', `reportPlaybackMetric(Conviva.Constants.Playback.RESOLUTION, ${this._player.videoWidth}, ${this._player.videoHeight}`);
            }
        };
        this._onPlayerPlay = () => {
            if (!this._player) { return; }
            if (this._isFirstContentPlay) {
                ...
                log('VideoAnalytics', `reportPlaybackRequested(${collectContentMetdata(this._player, contentMetadataReceiver(this._player.source))})`);
            }
        ...
        this._onPlayerPlaying = () => {
            ...
            if (!this._isPlayingAd && this._convivaVideoAnalytics) {
                ...
                log('VideoAnalytics', `reportPlaybackMetric(Conviva.Constants.Playback.PLAYER_STATE, Conviva.Constants.PlayerState.PLAYING)`);
            }
        };
        this._onPlayerPause = () => {
            if (!this._player) { return; }
            if (!this._isPlayingAd && this._convivaVideoAnalytics) {
                ...
                log('VideoAnalytics', `reportPlaybackMetric(Conviva.Constants.Playback.PLAYER_STATE, Conviva.Constants.PlayerState.PAUSED)`);
            }
        };
        this._onPlayerWaiting = () => {
            ...
            if (!this._isPlayingAd && this._convivaVideoAnalytics) {
                ...
                log('VideoAnalytics', `reportPlaybackMetric(Conviva.Constants.Playback.PLAYER_STATE, Conviva.Constants.PlayerState.BUFFERING)`);
            }
        };
        this._onPlayerSeeking = () => {
            ...
            if (!this._isPlayingAd && this._convivaVideoAnalytics) {
                ...
                log('VideoAnalytics', `reportPlaybackMetric(Conviva.Constants.Playback.SEEK_STARTED)`);
            }
        };
        this._onPlayerSeeked = () => {
            ...
            if (!this._isPlayingAd && this._convivaVideoAnalytics) {
                ...
                log('VideoAnalytics', `reportPlaybackMetric(Conviva.Constants.Playback.SEEK_ENDED)`);
            }
        };
        this._onPlayerError = () => {
            if (!this._player) { return; }
            ...
            log('VideoAnalytics', `reportPlaybackFailed('Fatal error occured')`);
            this._convivaVideoAnalytics.release();
            log('VideoAnalytics', `release()`);
            this._convivaVideoAnalytics = undefined;
            Conviva.Analytics.release();
            log('Conviva.Analytics', `release()`);
        ...
        this._onSegmentNotFound = () => {
            ...
            log('VideoAnalytics', `reportPlaybackError('A Video Playback Failure has occurred: Segment not found', Conviva.Constants.ErrorSeverity.FATAL)`);
        };
        this._reportManifestOffline = () => {
            ...
            log('VideoAnalytics', `reportPlaybackError('A Video Playback Failure has occurred: Waiting for the manifest to come back online',Conviva.Constants.ErrorSeverity.FATAL)`);
        };
        ...
        this._onSourceChange = () => {
            if (!this._player) { return; }
            if (this._currentSource === this._player.source) { return; }
            if (this._convivaVideoAnalytics) {
                this._onPlayerEmptied();
            }
        ...
        this._onPlayerEnded = () => {
            ...
            if (!this._isPlayingAd && this._convivaVideoAnalytics) {
                ...
                log('VideoAnalytics', `reportPlaybackMetric(Conviva.Constants.Playback.PLAYER_STATE, Conviva.Constants.PlayerState.STOPPED)`);
                if (this._allAdsCompleted) {
                    this._convivaVideoAnalytics.reportPlaybackEnded();
                    log('VideoAnalytics', `reportPlaybackEnded()`);
                    this._convivaVideoAnalytics.release();
                    log('VideoAnalytics', `release()`);
                    this._convivaVideoAnalytics = undefined;
                    Conviva.Analytics.release();
        ...
        this._onAdStopped = () => {
            ...
            if (this._convivaAdAnalytics) {
                ...
                log('AdAnalytics', `reportAdMetric(Conviva.Constants.Playback.PLAYER_STATE, Conviva.Constants.PlayerState.STOPPED)`);
                this._convivaAdAnalytics.reportAdSkipped();
                log('AdAnalytics', `reportAdSkipped()`);
                this._convivaAdAnalytics.release();
                log('AdAnalytics', `release()`);
                this._convivaAdAnalytics = undefined;
            }
        ...
        this._onAdBuffering = () => {
            ...
            if (this._convivaAdAnalytics) {
                ...
                log('AdAnalytics', `reportAdMetric(Conviva.Constants.Playback.PLAYER_STATE, Conviva.Constants.PlayerState.BUFFERING)`);
            }
        };
        this._onAdBreakBegin = () => {
            ...
            if (this._convivaVideoAnalytics && this._player.ads) {
                if (this._isAdBreakEnabled) {
                    ...
                    log('VideoAnalytics', `reportAdBreakStarted(Conviva.Constants.AdType.CLIENT_SIDE, Conviva.Constants.AdPlayer.CONTENT, ${convivaAdBreakInfo})`);
                    this._adBreakStartSent = true;
                }
                else {
                    this._convivaVideoAnalytics.reportPlayerInFocus(false);
                    log('VideoAnalytics', `reportPlayerInFocus(false)`);
                }
            }
        };
        this._onAdBreakEnd = () => {
            ...
            if (this._convivaVideoAnalytics) {
                if (this._isAdBreakEnabled) {
                    ...
                    if (!this._isPlayingPostRoll) {
                        ...
                        log('VideoAnalytics', `reportPlayerInFocus(true)`);
                    }
                }
                if (this._isPlayingPostRoll) {
                    this._convivaVideoAnalytics.reportPlaybackEnded();
            ...
            if (currentLinearAd) {
                if (this._adStartSent) {
                    if (this._convivaAdAnalytics) {
                        ...
                        log('AdAnalytics', `reportAdMetric(Conviva.Constants.Playback.PLAYER_STATE, Conviva.Constants.PlayerState.PLAYING)`);
                    }
                }
                else {
                    if (!this._convivaAdAnalytics) {
                    ...
                    if (this._convivaAdAnalytics) {
                        ...
                        log('AdAnalytics', `setAdInfo(${adMetadata})`);
                        this._convivaAdAnalytics.reportAdLoaded();
                        log('AdAnalytics', `reportAdLoaded()`);
                        this._convivaAdAnalytics.reportAdStarted();
                        log('AdAnalytics', `reportAdStarted()`);
                        ...
                        log('AdAnalytics', `reportAdMetric(Conviva.Constants.Playback.RESOLUTION, ${this._player.videoWidth}, ${this._player.videoHeight})`);
                        this._convivaAdAnalytics.reportAdMetric(Conviva.Constants.Playback.BITRATE, currentLinearAd.bitrate || 0);
                        log('AdAnalytics', `reportAdMetric(Conviva.Constants.Playback.BITRATE, ${currentLinearAd.bitrate || 0})`);
                    }
                }
            }
        };
        this._onAdsPlayerPause = () => {
            ...
            if (currentLinearAd && this._convivaAdAnalytics) {
                ...
                log('AdAnalytics', `reportAdMetric(Conviva.Constants.Playback.PLAYER_STATE, Conviva.Constants.PlayerState.PAUSED)`);
            }
        };
        this._onAdEnd = () => {
            if (!this._player || !this._player.ads) {
        ...
        this._onAdsError = (event) => {
            ...
            if (this._convivaAdAnalytics) {
                ...
                log('AdAnalytics', `reportAdFailed(${event.message || 'Ad Request Failed'})`);
                this._convivaAdAnalytics.release();
                log('AdAnalytics', `release()`);
                this._convivaAdAnalytics = undefined;
            }
    ...
    _initConvivaClient(convivaConfiguration) {
        ...
        log('Conviva.Analytics', `init(${convivaConfiguration[Conviva.Constants.CUSTOMER_KEY]}, CONVIVA_CALLBACK_FUNCTIONS, ${convivaConfiguration})`);
        Conviva.Analytics.setDeviceMetadata(collectDeviceMetadata());
        log('Conviva.Analytics', `setDeviceMetadata(${collectDeviceMetadata()})`);
        if (!this._convivaVideoAnalytics) {
            // Create a monitoring session for content
            this._convivaVideoAnalytics = Conviva.Analytics.buildVideoAnalytics();
            log('Conviva.Analytics', `buildVideoAnalytics()`);
            this._convivaVideoAnalytics.setPlayerInfo(collectPlayerInfo());
            log('VideoAnalytics', `setPlayerInfo(${collectPlayerInfo()})`);
            this._convivaVideoAnalytics.setCallback(() => {
                if (this._convivaVideoAnalytics) {
                    if (this._player) {
                        ...
                        log('VideoAnalytics', `reportPlaybackMetric(Conviva.Constants.Playback.PLAY_HEAD_TIME, ${this._player.currentTime * 1000})`);
                        ...
                        log('VideoAnalytics', `reportPlaybackMetric(Conviva.Constants.Playback.BUFFER_LENGTH, ${calculateBufferLength(this._player)})`);
                        this._convivaVideoAnalytics.reportPlaybackMetric(Conviva.Constants.Playback.RESOLUTION, this._player.videoWidth, this._player.videoHeight);
                        log('VideoAnalytics', `reportPlaybackMetric(Conviva.Constants.Playback.RESOLUTION, ${this._player.videoWidth}, ${this._player.videoHeight})`);
                        ...
                        if (activeQuality) {
                            if (!isNaN(activeQuality.bandwidth)) {
                                ...
                                log('VideoAnalytics', `reportPlaybackMetric(Conviva.Constants.Playback.BITRATE, ${activeQuality.bandwidth / 1000})`);
                            }
                            ...
                            if (!isNaN(activeQuality.frameRate)) {
                                ...
                                log('VideoAnalytics', `reportPlaybackMetric(Conviva.Constants.Playback.RENDERED_FRAMERATE, ${activeQuality.frameRate})`);
                            }
                        }
                    }
                }
            });
            log('VideoAnalytics', `setCallback(...)`);
            this._isFirstContentPlay = true;
            this._isAdBreakEnabled = true;
            this._contentPlaybackEnded = false;
            this._allAdsCompleted = true;
